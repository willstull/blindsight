...
from src.services.case.pivot import (
    save_pivot, list_pivots, get_pivot,
    query_pivot_timeline, find_event_clusters,
)
...
def create_case_server(cases_dir: Path, logger: logging.Logger) -> FastMCP:
    ...
    @server.tool()
    async def save_investigation_pivot_tool(
        case_id: str,
        label: str,
        event_ids: list[str],
        entity_ids: list[str],
        ...
        """Save an investigation pivot -- a named evidence slice for revisiting."""
        request_id = generate_ulid()
        err = _validate_case_id(case_id)
        if err:
            return _error_envelope(request_id, "invalid_case_id", err)
        ...
        conn = db_result.ok()
        try:
            pivot_id = generate_ulid()
            result = save_pivot(
                logger, conn, pivot_id, case_id, label, description,
                event_ids, entity_ids, relationship_ids,
                focal_entity_ids=focal_entity_ids,
                created_from_tool_call_ids=created_from_tool_call_ids,
            )
            if result.is_err():
                return _error_envelope(request_id, "save_pivot_failed", str(result.err()))
            return _success_envelope(request_id, results=[result.ok()])
        finally:
            conn.close()

    @server.tool()
    async def list_investigation_pivots_tool(case_id: str) -> dict:
        """List all investigation pivots for a case."""
        request_id = generate_ulid()
        err = _validate_case_id(case_id)
        if err:
            return _error_envelope(request_id, "invalid_case_id", err)
        ...
        conn = db_result.ok()
        try:
            result = list_pivots(logger, conn, case_id)
            if result.is_err():
                return _error_envelope(request_id, "query_failed", str(result.err()))
            return _success_envelope(request_id, results=result.ok())
        finally:
    ...
    @server.tool()
    async def get_investigation_pivot_tool(case_id: str, pivot_id: str) -> dict:
        """Fetch a single investigation pivot by ID."""
        request_id = generate_ulid()
        err = _validate_case_id(case_id)
        if err:
            return _error_envelope(request_id, "invalid_case_id", err)
        ...
        try:
            ...
            pivot = result.ok()
            if pivot is None:
                return _error_envelope(request_id, "pivot_not_found", f"Pivot '{pivot_id}' not found")
            return _success_envelope(request_id, results=[pivot])
        finally:
            conn.close()

    @server.tool()
    async def query_pivot_timeline_tool(
        case_id: str,
        pivot_id: str,
        limit: int = 100,
    ) -> dict:
        ...
        conn = db_result.ok()
        try:
            result = query_pivot_timeline(logger, conn, pivot_id, limit)
            if result.is_err():
                return _error_envelope(request_id, "query_failed", str(result.err()))
            return _success_envelope(request_id, events=result.ok())
        finally:
    ...
    @server.tool()
    async def find_event_clusters_tool(
        case_id: str,
        pivot_id: str,
        ...
        conn = db_result.ok()
        try:
            result = find_event_clusters(logger, conn, pivot_id, window_minutes, min_events)
            if result.is_err():
                return _error_envelope(request_id, "query_failed", str(result.err()))
            return _success_envelope(request_id, results=result.ok())
        finally:
